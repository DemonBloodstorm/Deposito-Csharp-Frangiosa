...
SELECT clienti2.nome AS Nome, COUNT(ORDINI.id) AS 'Numero Ordini', SUM(ordini.importo) AS 'Importo'
FROM ordini
INNER JOIN clienti2 ON clienti2.id = ordini.id_cliente
GROUP BY clienti2.nome

/* Elenca i clienti che non hanno effettuato alcun ordine */
SELECT clienti2.nome, clienti2.città AS Città 
FROM clienti2
LEFT JOIN ordini ON  ordini.id_cliente = clienti2.id
WHERE ordini.id IS NULL

/* Elenca gli ordini che non hanno un clienti associato */
SELECT ordini.id AS 'ID Ordine', ordini.data_ordine AS 'Data Ordine', ordini.importo AS 'Importo', nome AS Nome
FROM clienti2
RIGHT JOIN ordini ON clienti2.id = ordini.id_cliente
WHERE clienti2.nome IS NULL
